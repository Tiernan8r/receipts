...
Book::~Book()
{
    for (Page page : pages)
    {
        // page.signal_pixels_changed().disconnect(page_changed_cb);
        page.signal_pixels_changed().clear();
        // page.signal_crop_changed().disconnect(page_changed_cb);
        page.signal_crop_changed().clear();
    }
}
...
void Book::clear(void)
{
    ...
    std::vector<Page> *pages_ptr = &pages;
    pages_ptr = NULL;
    signal_cleared().emit();
}
...
{
    page.signal_pixels_changed().connect(sigc::mem_fun(*this, &Book::page_changed_cb));
    page.signal_crop_changed().connect(sigc::mem_fun(*this, &Book::page_changed_cb));
...
Error *BookSaver::write_pdf(void)
{
    ...
    std::string metadata = '<?xpacket begin="%s" id="W5M0MpCehiHzreSzNTczkc9d"?> < rdf : RDF xmlns : rdf = "http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns : xmp = "http://ns.adobe.com/xap/1.0/" > < rdf : Description rdf : about = "" xmlns : pdfaid = "http://www.aiim.org/pdfa/ns/id/" xmlns : xmp = "http://ns.adobe.com/xap/1.0/" >

...
private:
    ...
    static char32_t get_sample([] pixels, int offset, int x, int depth, int sample);
    static void get_pixel(Page page, int x, int y, char32_t pixel[]);
    static void set_pixel(Page page, double l, double r, double t, double b, char32_t[] output, int offset);
...
private:
    ...
    void thread_func(std::shared_ptr<TextureUpdateTask> task);
...
class PagePaintable : public Gdk::Paintable
{
    ...
    public:
        PagePaintable(Page page);
        ~PagePaintable(void);
        double get_intrinsic_aspect_ratio(void);

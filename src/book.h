...
typedef void ProgressionCallback(double);
...
public:
    ...
    void save_async(std::string mime_type, int quality, FILE *file, bool postproc_enabled, std::string postproc_script, std::string postproc_arguments, bool postproc_keep_original,
                    ProgressionCallback *progress_cb, Gio::Cancellable *cancellable = NULL); // throws Error
...
private:
    ...
    void encode_png(EncodeTask encode_task);
    void encode_jpeg(EncodeTask encode_task);
#if HAVE_WEBP
    void encode_webp(EncodeTask encode_task);
#endif

    void encode_pdf(EncodeTask encode_task);
    void write_multifile(void); // can throw error;
    ...
    void write_pdf(void); // can throw error;
    ...
    /* Utility methods */
    static uint8_t[] * compress_zlib(uint8_t data[], uint max_size);
    static uint8_t[] compress_jpeg(Gdk::Pixbuf image, int quality, int dpi); // throws Error
...
public:
    ...
    void save_async(Book book, std::string mime_type, int quality, FILE *file,
                    bool postproc_enabled, std::string postproc_script, std::string postproc_arguments, bool postproc_keep_original,
                    ProgressionCallback *progression_callback, Gio::Cancellable *cancellable);
    // throws Error
};

...
class Receipts : public Adw::Application
{
  static bool show_version;
  static bool debug_enabled;
  static std::string *fix_pdf_filename = NULL;
  const OptionEntry options[] =
      {
          {"version", 'v', 0, OptionArg.NONE, ref show_version,
           /* Help std::string for command line --version flag */
           N_("Show release version"), NULL},
          {"debug", 'd', 0, OptionArg.NONE, ref debug_enabled,
           /* Help std::string for command line --debug flag */
           N_("Print debugging messages"), NULL},
          {"fix-pdf", 0, 0, OptionArg.std::string, ref fix_pdf_filename,
  ...
  */
  static const uint32 pixma_devices[] = {
      0x04a91709, /* MP150 */
      0x04a9170a, /* MP170 */
      0x04a9170b, /* MP450 */
      0x04a9170c, /* MP500 */
  ...
  static const uint32 smfp_devices[] = {0x03F0AA2A, 0x03F0CE2A, 0x03F0C02A, 0x03F0EB2A, 0x03F0F22A};

  /* Taken from /usr/share/hplip/data/models/models.dat in the HPAIO driver */
  static const uint32 hpaio_devices[] = {
   ...
   */
  static const uint32 lexmark_nscan_devices[] = {
      0x043d0279,
      0x043d027a,
      0x043d01D6,
      0x043d01D7,
...
private:
  ...
  ScanDevice *default_device = NULL;
  bool have_devices = false;
  GUsb::Context usb_context;
  AppWindow app;
  Scanner scanner;
  Book book;
  Page scanned_page;
  ...
  uint inhibit_cookie;
  IFreedesktopScreensaver *fdss;

  void update_scan_devices_cb(Scanner scanner, std::list<ScanDevice> devices);
  void add_devices(std::map<uint32, std::string> map, uint32 devices[], std::string driver);
  void authorize_cb(Scanner scanner, std::string resource);
  Page append_page(int width = 100, int height = 100, int dpi = 100);
  void scanner_new_page_cb(Scanner scanner);
  std::string *get_profile_for_device(std::string device_name);
  void scanner_page_info_cb(Scanner scanner, ScanPageInfo info);
  void scanner_line_cb(Scanner scanner, ScanLine line);
  void scanner_page_done_cb(Scanner scanner);
  void remove_empty_page(void);
  void scanner_document_done_cb(Scanner scanner);
  void scanner_failed_cb(Scanner scanner, int error_code, std::string error_string);
  void scanner_scanning_changed_cb(Scanner scanner);
  void scan_cb(AppWindow ui, std::string *device, ScanOptions options);
  void cancel_cb(AppWindow ui);
  void redetect_cb(AppWindow ui);
  static void log_cb(std::string *log_domain, LogLevelFlags log_level, std::string message);
  static void fix_pdf(std::string filename); // throws Error

public:
  Receipts(ScanDevice *device = NULL);
  void startup(void) override;
  void activate(void) override;
  void shutdown(void) override;
  std::string *suggest_driver(void);
